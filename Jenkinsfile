  ...
  stages {
    ...
    stage('Build') {
      steps {
        ...
        sh "docker -H ssh://${BUILD_HOST} container stop $(docker -H ssh://${BUILD_HOST} ps -q)"
        sh "docker -H ssh://${BUILD_HOST} container prune -f"
        sh "docker -H ssh://${BUILD_HOST} network prune -f"
